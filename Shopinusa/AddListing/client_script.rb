def addlisting(data)
  ...
  button = @browser.button(:id=>"ct100_MainContent_subButton")
  field = @browser.text_field(:id => "recaptcha_response_field")
  image = @browser.div(:id=>"recaptcha_image").image
  enter_captcha(button,field,image,"What does your company do?")
  ...
  if (tries -= 1) > 0
    puts "Shopinusa/AddListing failed. Retrying #{tries} more times."
    puts "Details: #{e.message}"
  else
    puts "Shopinusa/AddListing failed. Out of retries. Quitting."
    raise e
  end
else
  puts "Shopinusa/AddListing succeeded!"
  true
end