...
def search_for_business(business)
  retries = 3
  begin
  @browser.goto 'http://www.crunchbase.com/'
  browser.text_field(:id => 'search_box').set 't1t'
  browser.button(:id => 'search_button').click
  browser.p(:text => 'Sorry, we could not find any search results for “t1t ”.').click

  @business_found = false

  sleep 2

  if @browser.text.include? "Sorry, we could not find any search results for"
    @business_found = false
  else
    @business_found = true
  end
